# webserver-oob_git.bb — single recipe for all Sitara devices
#
# Copy to:
#   meta-ti-foundational/recipes-demos/webserver-oob/webserver-oob_git.bb
# Also copy webserver-oob-npm.inc (generated by tools/generate-inc.js) to same dir.
#
# Device folder names match Yocto SOC_FAMILY overrides:
#   am335x-evm  → devices/am335x/   (SOC_FAMILY: ti33x — use machine name explicitly)
#   am62xx      → devices/am62xx/   (covers am62xx-evm, am62xxsip-evm, am62xx-lp-evm)
#   am62pxx     → devices/am62pxx/  (covers am62pxx-evm)
#   am62lxx     → devices/am62lxx/  (covers am62lxx-evm)
#   am62dxx     → devices/am62dxx/  (covers am62dxx-evm)

SUMMARY = "Webserver OOB Demo — multi-device"
...
# webserver-oob-npm.inc auto-generated by tools/generate-inc.js.
# Provides NPM_SRC_URI and NPM_PACKAGE_MAP variables.
require webserver-oob-npm.inc

SRC_URI = " \
    git://github.com/paresh-bhagat/webserver-oob-demo-pb.git;protocol=https;branch=clean-history \
...
RDEPENDS:${PN}:am62dxx      = "nodejs ti-tidl-osrt nnstreamer analytics-demo-data"
...
# linux_app dirs are outside S — add explicit prefix map so debug info paths get remapped
...
do_compile() {
    ...
    if [ -d ${WEBSERVER_ROOT}/devices/${DEVICE_ID}/linux_app ]; then
        # Evaluate pkg-config here (recipe shell has PKG_CONFIG_PATH set by Yocto)
        # and pass results as make variables so the Makefile's $(shell pkg-config ...)
        # is overridden — necessary for cross-compilation where the Makefile's subshell
        # may not inherit PKG_CONFIG_PATH correctly.
        GST_CFLAGS="$(pkg-config --cflags gstreamer-1.0 2>/dev/null)"
        GST_LIBS="$(pkg-config --libs gstreamer-1.0 2>/dev/null)"
        oe_runmake -C ${WEBSERVER_ROOT}/devices/${DEVICE_ID}/linux_app \
            "GST_CFLAGS=${GST_CFLAGS}" "GST_LIBS=${GST_LIBS}"
    fi
}
...
do_install:append:am335x-evm() {
    install -m 0755 ${WEBSERVER_ROOT}/devices/am335x/linux_app/audio_utils \
        ${D}${bindir}/audio_utils
}

do_install:append:am62xx() {
    install -m 0755 ${WEBSERVER_ROOT}/devices/am62xx/linux_app/audio_utils \
        ${D}${bindir}/audio_utils
}

do_install:append:am62pxx() {
    install -m 0755 ${WEBSERVER_ROOT}/devices/am62pxx/linux_app/audio_utils \
        ${D}${bindir}/audio_utils
    install -m 0755 ${WEBSERVER_ROOT}/devices/am62pxx/linux_app/speech_utils \
        ${D}${bindir}/speech_utils
    install -m 0755 ${WEBSERVER_ROOT}/tools/make_static_onnx.py \
        ${D}${bindir}/make_static_onnx
}

# Generate en_v5_static.onnx from en_v5.onnx on first boot.
# en_v5.onnx ships with analytics-demo-data (RDEPENDS) and is only
# available on-target, so the conversion must happen post-install.
# Requires python3-onnx on the target (add to RDEPENDS below if available
# in the layer; otherwise install manually: pip3 install onnx).
pkg_postinst_ontarget:${PN}:am62pxx() {
#!/bin/sh
MODEL_DIR="/usr/share/oob-demo-assets/models"
SRC="$MODEL_DIR/en_v5.onnx"
DST="$MODEL_DIR/en_v5_static.onnx"
if [ -f "$SRC" ] && [ ! -f "$DST" ]; then
    make_static_onnx "$SRC" "$DST" \
        && echo "webserver-oob: generated $DST" \
        || echo "webserver-oob: WARNING: could not generate en_v5_static.onnx - speech-to-text will not work. Run: python3 /usr/bin/make_static_onnx $SRC $DST"
fi
}

do_install:append:am62lxx() {
    install -m 0755 ${WEBSERVER_ROOT}/devices/am62lxx/linux_app/audio_utils \
        ${D}${bindir}/audio_utils
}
