...
# webserver-oob-frontend-npm.inc auto-generated by tools/generate-frontend-inc.js.
# Provides FRONTEND_NPM_SRC_URI/FRONTEND_NPM_PACKAGE_MAP for the am62dxx
# Vue/Vuetify build. Only required for am62dxx so other devices never even
# parse these variables.
FRONTEND_NPM_INC = ""
FRONTEND_NPM_INC:am62dxx = "webserver-oob-frontend-npm.inc"
require ${FRONTEND_NPM_INC}
...
SRCREV = "80bf2c0f391e96430d8bad25c660d0e5482b572e"
...
# am62dxx uses ALSA (no GStreamer/TFLite/NNStreamer); override the default RDEPENDS
RDEPENDS:${PN}:am62dxx = "nodejs alsa-lib"
...
# am62dxx: native node is required to run Vite during the Vue frontend build
DEPENDS:append:am62dxx = " nodejs-native"
...
python do_npm_unpack() {
    ...
    def extract(npm_map_var, target_dir, label):
        npm_map = d.getVar(npm_map_var) or ""

        # Parse package map into list of (install_path, tarball) tuples
        packages = [e.split('=', 1) for e in npm_map.split() if '=' in e]
        if not packages:
            bb.note("No %s npm packages to extract" % label)
            return

        bb.note("Preparing to extract %d %s npm packages..." % (len(packages), label))
        ...
        if missing:
            bb.fatal("Missing %s npm tarballs (%d):\n  %s" % (label, len(missing), "\n  ".join(missing)))

        # Extract each package
        for idx, (install_path, tarball) in enumerate(packages, 1):
            target = os.path.join(target_dir, install_path)
            tarball_path = os.path.join(dl_dir, tarball)

            bb.utils.mkdirhier(target)

            cmd = ["tar", "xzf", tarball_path, "--strip-components=1", "-C", target]
            try:
                subprocess.check_output(cmd, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                bb.fatal("Failed to extract %s package %d/%d (%s -> %s): %s\nOutput: %s"
                         % (label, idx, len(packages), tarball, install_path, str(e), e.output.decode('utf-8', errors='replace')))

            # Progress indicator for every 20 packages
            if idx % 20 == 0:
                bb.note("Progress: %d/%d %s packages extracted" % (idx, len(packages), label))

        bb.note("Successfully extracted %d %s npm packages into %s" % (len(packages), label, target_dir))

    extract("NPM_PACKAGE_MAP", d.getVar("S"), "backend")
}
addtask npm_unpack after do_unpack before do_patch

# am62dxx ships a Vue/Vuetify frontend with its own, separate node_modules
# tree.
python do_npm_unpack:append:am62dxx() {
    frontend_dir = os.path.join(d.getVar("WEBSERVER_ROOT"), "frontend")
    extract("FRONTEND_NPM_PACKAGE_MAP", frontend_dir, "frontend")
}
...
def get_active_demos(d):
    ...
    device_json_path = os.path.join(d.getVar("WEBSERVER_ROOT"), "devices", d.getVar("DEVICE_ID"), "device.json")
    try:
        with open(device_json_path) as f:
            device = json.load(f)
    except (IOError, ValueError) as e:
        bb.warn("Could not read %s (%s); no per-device demos will be installed" % (device_json_path, e))
        return ""

    return " ".join(device.get("demos", []))

python do_set_active_demos(){
    if d.getVar("DEVICE_ID") == "am62dxx":
        d.setVar("ACTIVE_DEMOS",get_active_demos(d))
}
do_install[prefuncs] += "set_active_demos"
...
# am62dxx: build Vue/Vuetify frontend using Vite (node_modules populated by do_npm_unpack)
do_compile:append:am62dxx() {
    # esbuild native binary requires execute permission after tarball extraction
    chmod +x ${WEBSERVER_ROOT}/frontend/node_modules/@esbuild/linux-x64/bin/esbuild
    ${STAGING_BINDIR_NATIVE}/node ${WEBSERVER_ROOT}/frontend/node_modules/vite/bin/vite.js build ${WEBSERVER_ROOT}/frontend
}
...
do_install() {
    ...
    # Preserve the previous demo payload on all non-AM62D devices.
    install -d ${D}${datadir}/${BPN}/demos
    if [ "${DEVICE_ID}" = "am62dxx" ]; then
        if [ -f ${WEBSERVER_ROOT}/demos/demo-coordinator.js ]; then
            install -m 0644 ${WEBSERVER_ROOT}/demos/demo-coordinator.js ${D}${datadir}/${BPN}/demos/
        fi
        for demo in ${ACTIVE_DEMOS}; do
            if [ -d ${WEBSERVER_ROOT}/demos/$demo ]; then
                cp $CP_ARGS ${WEBSERVER_ROOT}/demos/$demo ${D}${datadir}/${BPN}/demos/
            fi
        done
    else
...
# am62dxx: DEVICE_CONFIG must be one level above app/ so server.js can derive
# deviceAppDir = path.dirname(DEVICE_CONFIG) + '/app' → /usr/share/webserver-oob/app/
# and find vue-dist/index.html inside it.
do_install:append:am62dxx() {
    # Reinstall device.json at parent of app/ (overrides the app/device.json placed by base task)
    install -m 0644 ${WEBSERVER_ROOT}/devices/${DEVICE_ID}/device.json \
    ...
    # Fix DEVICE_CONFIG in the conf file to the relocated path
    sed -i 's|^DEVICE_CONFIG=.*$|DEVICE_CONFIG='"${datadir}/${BPN}/device.json"'|' \
        ${D}${sysconfdir}/webserver-oob.conf
}
