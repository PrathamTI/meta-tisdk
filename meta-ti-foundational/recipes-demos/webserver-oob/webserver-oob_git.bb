...
python do_npm_unpack() {
    ...
    # tree (see FRONTEND_NPM_INC above for why it can't share the backend's).
...
do_compile:append:am62dxx() {
    ...
    node ${WEBSERVER_ROOT}/frontend/node_modules/vite/bin/vite.js build ${WEBSERVER_ROOT}/frontend
}
...
do_install() {
    ...
    # Install only the demos relevant device actually uses
    ...
    for demo in ${@get_active_demos(d)}; do
...
do_install:append:am62dxx() {
    ...
    install -m 0755 ${WEBSERVER_ROOT}/devices/${DEVICE_ID}/linux_app/speech_utils \
        ${D}${bindir}/speech_utils
