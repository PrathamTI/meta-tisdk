...
# Provides FRONTEND_NPM_SRC_URI and FRONTEND_NPM_PACKAGE_MAP for Vue/Vuetify build (am62dxx).
...
# Extract Vue/Vuetify frontend npm tarballs into frontend/node_modules/ (am62dxx only).
# Runs after do_npm_unpack so the server's node_modules are already in place.
python do_frontend_npm_unpack() {
    import subprocess
    import os

    # No-op for non-am62dxx devices (FRONTEND_NPM_SRC_URI not added to SRC_URI for them)
    if d.getVar("DEVICE_ID") != "am62dxx":
        return

    frontend_dir = os.path.join(d.getVar("WEBSERVER_ROOT"), "frontend")
    dl_dir = d.getVar("DL_DIR")
    npm_map = d.getVar("FRONTEND_NPM_PACKAGE_MAP") or ""

    packages = [e.split('=', 1) for e in npm_map.split() if '=' in e]
    if not packages:
        bb.note("No frontend npm packages to extract")
        return

    bb.note("Extracting %d frontend npm packages into %s/node_modules/" % (len(packages), frontend_dir))

    missing = [t for _, t in packages if not os.path.exists(os.path.join(dl_dir, t))]
    if missing:
        bb.fatal("Missing frontend npm tarballs (%d):\n  %s" % (len(missing), "\n  ".join(missing)))

    for idx, (install_path, tarball) in enumerate(packages, 1):
        target_dir = os.path.join(frontend_dir, install_path)
        tarball_path = os.path.join(dl_dir, tarball)
        ...
        try:
            ...
            bb.fatal("Failed to extract frontend package %d/%d (%s -> %s): %s\nOutput: %s"
                     % (idx, len(packages), tarball, install_path, str(e), e.output.decode('utf-8', errors='replace')))

        if idx % 20 == 0:
            bb.note("Progress: %d/%d frontend packages extracted" % (idx, len(packages)))

    bb.note("Successfully extracted %d frontend npm packages" % len(packages))
}
addtask frontend_npm_unpack after do_npm_unpack before do_compile
...
# am62dxx: build Vue/Vuetify frontend using Vite (node_modules populated by do_frontend_npm_unpack)
do_compile:append:am62dxx() {
    # esbuild native binary requires execute permission after tarball extraction
    chmod +x ${WEBSERVER_ROOT}/frontend/node_modules/@esbuild/linux-x64/bin/esbuild || true

    # Run Vite inside a subshell so process.cwd() == frontend/ when Vite resolves
    # vite.config.js (which uses relative outDir ../devices/am62dxx/app/vue-dist)
    (
        cd ${WEBSERVER_ROOT}/frontend
